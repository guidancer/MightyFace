...
const initMightyService = async (offset_value, limit_value) => {
    try {
        await axios.get(
            "/admin/all_user_auth/?offset=" + offset_value + "&limit=" + limit_value).then(res => {
...
const fetchUpgrade = async (selected_id) => {
    try {
        await axios.get(
            "/admin/upgrade_user/?selected_id=" + selected_id).then(res => {
                console.log(res.data)                //请求成功后执行函数
...
const fetchBan = async (selected_id) => {
    try {
        await axios.get(
            "/admin/ban_user/?selected_id=" + selected_id).then(res => {
                console.log(res.data)                //请求成功后执行函数
                fetchMightyUser(currentPage.value, 13)
            }).catch(err => {
                console.log("维护用户信息失败")
...
function upgradeIt(event) {
    fetchUpgrade(parseInt(event.target.parentNode.parentNode.childNodes[0].innerText))
}
function banIt(event) {
    fetchBan(parseInt(event.target.parentNode.parentNode.childNodes[0].innerText))
}
...
function showOnline(event) {
    const status_online = document.getElementsByName("离线")
    console.log(status_online)
    console.log(status_online[0])
    for (let i = 0; i < status_online.length; i++) {
        console.log(status_online[i])
    };
    componentKey.value += 1
                    ...
                    <table class="w-full mt-12 text-md text-left rtl:text-right text-gray-300 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in items" :key="index" class="">
                                ...
                                <td class="px-6 py-4">
                                    <div
                                        :class="[checkBan(item.user_status) ? 'rounded-full px-5 w-20 py-2 text-md font-semibold bg-emerald-500' : 'rounded-full px-5 w-20 py-2 text-md font-semibold bg-red-500']">
                                        <span
                                            :class="[checkOffline(item.user_status) ? 'text-gray-900' : 'text-gray-600']">
                                            {{ item.user_status }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <button @click="upgradeIt"
