...
const real_score = ref(null)
const real_score_value = ref(null)
const fake_score = ref(null)
const fake_score_value = ref(null)
const router = useRouter()
const user_name = ref(null)
const user_email = ref(null)
const currentTab = ref(null)
const passive_detection_real = ref(null)
const passive_detection_msgbox = ref(null)
const passive_detection_image_repo = ref(null)
...
function agImgRepo(event) {
    passive_detection_image_repo.value.click()
    agInitMightyImageRepo(1, 8)
}
function agConfirmChecked() {
    passive_detection_real.value.src = agPreview.value.src
    passive_detection_real.value.style = "height:410px;weight:410px"
    agCurrentPage.value = 1
}
const passiveFetchDetection = async (image_id_value) => {
    try {
        await axios({
            method: "post",
            url: '/user/passive_defense',
            headers: {
            ...
            if (res.message == "被动防御成功!") {
                fake_score.value.style = "width:" + res.data.fake_score * 100 + "%"
                real_score.value.style = "width:" + res.data.real_score * 100 + "%"
                fake_score_value.value.innerText = res.data.fake_score * 100 + "%"
                real_score_value.value.innerText = res.data.real_score * 100 + "%"
            }
            else {
                alert(res.message)
            }
        }).catch(err => {                   //请求错误后执行函
            console.log("passive_detection_decode请求错误")
        })
    } catch (error) {
        console.error('passive_detection_decode错误', error);
    }
};
function passiveDetection() {
    let real_image_src = passive_detection_real.value.src
    ...
    passiveFetchDetection(image_id_value)
}
//整体方法
function mightySocketOpen() {
    console.log("MightySocketOpen")
    passive_detection_msgbox.value.value = ""
}
...
function mightySocketMessage(event) {
    ...
    if (received_msg.sender_type == "passive_defense") {
        passive_detection_msgbox.value.value = passive_detection_msgbox.value.value + received_msg.sender_msg + "\n"
    }
}
            ...
            <div class="items-center justify-center mt-8 mb-12">
                <div>
                    <div class="w-3/12 inline-block">
                        <div class="mt-3 ml-3 bg-white rounded-2xl shadow-xl">
                            <img class="w-auto h-auto rounded-2xl" src="../../assets/people_1.jpg"
                                ref="passive_detection_real" @click="agImgRepo" />
                        </div>
                    </div>
                    <div class="w-2/12 inline-block">
                        <div class="rounded-2xl place-content-center place-items-center">
                            <button data-modal-target="active-defense-modal" data-modal-toggle="active-defense-modal"
                                ref="passive_detection_image_repo"
                    ...
                    <div class="w-2/12 inline-block">
                        <div class="mb-20 rounded-2xl place-content-center place-items-center pl-12">
                            <img class="w-full h-auto rounded-2xl" src="../../assets/big_detect.png" />
                        </div>
                    </div>
                    <div class="w-2/12 inline-block">
                        <div class="mb-24 rounded-2xl place-content-center place-items-center pl-12">
                            <img class="w-auto h-auto rounded-2xl" src="../../assets/southeast.png" />
                        </div>
                    </div>
                    <div class="w-3/12 inline-block">
                        <div class="mt-3 mr-3 ">
                            <div class="flex justify-between mb-1">
                                <span class="text-base font-medium text-emerald-700 dark:text-white">图像为真概率</span>
                                <span class="text-md font-medium text-emerald-700 dark:text-white"
                                    ref="real_score_value">50%</span>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 mb-6">
                                <div class="bg-emerald-600 h-2.5 rounded-full" style="width: 50%" ref="real_score">
                                </div>
                            </div>
                            <div class="flex justify-between mb-1">
                                <span class="text-base font-medium text-red-700 dark:text-white">图像为假概率</span>
                                <span class="text-md font-medium text-red-700 dark:text-white"
                                    ref="fake_score_value">50%</span>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 mb-36 ">
                                <div class="bg-red-600 h-2.5 rounded-full" style="width: 50%" ref="fake_score"></div>
                            </div>
                            <div class=" text-blue-100">我是不可见的定位符</div>
                        </div>
                    </div>
                ...
                <div>
                    <div class="w-8/12 inline-block ml-3">
                        ...
                        <textarea id="message" rows="16" ref="passive_detection_msgbox"
                    ...
                    <div class="inline-block" style="width:32%">
                        <div class="mt-3 rounded-2xl ml-32 mr-3">
                            <img @click="passiveDetection" class="w-full h-auto rounded-2xl"
                                src="../../assets/passive_detection.png" />
                        </div>
                    </div>
                </div>
            </div>
