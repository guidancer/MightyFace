...
const active_detection_real = ref(null)
const active_detection_res = ref(null)
const active_detection_msgbox = ref(null)
const active_detection_image_repo = ref(null)
...
function agImgRepo(event) {
    active_detection_image_repo.value.click()
    agInitMightyImageRepo(1, 8)
}
function agConfirmChecked() {
    active_detection_real.value.src = agPreview.value.src
    active_detection_real.value.style = "height:410px;weight:410px"
}
const activeFetchDetectionEncode = async (image_id_value) => {
    try {
        await axios({
            method: "post",
            url: '/user/active_detection_encode',
            headers: {
            ...
            if (res.message == "主动取证编码成功!") {
                active_detection_res.value.src = "data:image/png;base64," + res.data
                active_detection_res.value.style = "width:410px;height:410px"
                active_detection_msgbox.value.value += '\n'
            }
            else {
                alert(res.message)
            }
        }).catch(err => {                   //请求错误后执行函
            console.log("active_detection_encode请求错误")
        })
    } catch (error) {
        console.error('active_detection_encode错误', error);
    }
};
function activeDetectionEncode() {
    let real_image_src = active_detection_real.value.src
    ...
    activeFetchDetectionEncode(image_id_value)
}
const activeFetchDetectionDecode = async (image_id_value) => {
    try {
        await axios({
            method: "post",
            url: '/user/active_detection_decode',
            headers: {
                "Content-Type": "application/json"
            },
            data: JSON.stringify({
            ...
            console.log(res)                //请求成功后执行函数
            if (res.message == "主动取证解码成功!") {
                active_detection_msgbox.value.value += "追踪器提取误码率为:" + res.data.error_rate_C * 100 + "%" + ",提取率为:" + (1 - res.data.error_rate_C) * 100 + "%",
                    active_detection_msgbox.value.value += '\n'
                active_detection_msgbox.value.value += "检测器提取误码率为:" + res.data.error_rate_RF * 100 + "%" + ",提取率为:" + (1 - res.data.error_rate_RF) * 100 + "%",
                    active_detection_msgbox.value.value += '\n'
                active_detection_msgbox.value.value += "追踪与提取分析误码率为:" + res.data.error_rate_Both * 100 + "%",
                    active_detection_msgbox.value.value += '\n'
                if (res.data.error_rate_C + res.data.error_rate_RF <= 0) {
                    active_detection_msgbox.value.value += "该图像一定为本系统主动取证后的图像"
                    active_detection_msgbox.value.value += '\n'
                } else if (res.data.error_rate_C <= 0.125) {
                    active_detection_msgbox.value.value += "该图像的原始图像极有可能为本系统主动取证后的图像"
                    active_detection_msgbox.value.value += '\n'
                } else if (res.data.error_rate_C <= 0.25) {
                    active_detection_msgbox.value.value += "该图像的原始图像有较大可能为本系统主动取证后的图像"
                    active_detection_msgbox.value.value += '\n'
                } else if (res.data.error_rate_C <= 0.375) {
                    active_detection_msgbox.value.value += "该图像的原始图像有可能为本系统主动取证后的图像"
                    active_detection_msgbox.value.value += '\n'
                } else if (res.data.error_rate_C <= 0.75) {
                    active_detection_msgbox.value.value += "该图像的原始图像不太可能为本系统主动取证后的图像"
                    active_detection_msgbox.value.value += '\n'
                } else if (res.data.error_rate_C < 1) {
                    active_detection_msgbox.value.value += "该图像的原始图像基本不可能为本系统主动取证后的图像"
                    active_detection_msgbox.value.value += '\n'
                } else {
                    active_detection_msgbox.value.value += "该图像的原始图像不可能为本系统主动取证后的图像"
                    active_detection_msgbox.value.value += '\n'
                }
                // if (res.data.error_rate_Both >= 0.5) {
                //     active_detection_msgbox.value.value += "该图像极有可能经过了伪造篡改"
                //     active_detection_msgbox.value.value += '\n'
                // } else if (res.data.error_rate_Both >= 0.25) {
                //     active_detection_msgbox.value.value += "该图像很有可能经过了伪造篡改"
                //     active_detection_msgbox.value.value += '\n'
                // } else if (res.data.error_rate_Both >= 0.1) {
                //     active_detection_msgbox.value.value += "该图像较有可能经过了伪造篡改"
                //     active_detection_msgbox.value.value += '\n'
                // } else if (res.data.error_rate_Both >= 0.01) {
                //     active_detection_msgbox.value.value += "该图像有可能经过了伪造篡改"
                //     active_detection_msgbox.value.value += '\n'
                // }
                // else if (res.data.error_rate_Both > 0.0) {
                //     active_detection_msgbox.value.value += "该图像经过了伪造篡改可能性较小"
                //     active_detection_msgbox.value.value += '\n'
                // } else {
                //     active_detection_msgbox.value.value += "该图像经过了伪造篡改可能性为0"
                //     active_detection_msgbox.value.value += '\n'
                // }
            }
            else {
            ...
            console.log("active_detection_decode请求错误")
        })
    } catch (error) {
        console.error('active_detection_decode错误', error);
    }
};
function activeDetectionDecode() {
    let real_image_src = active_detection_real.value.src
    console.log(real_image_src)
    let real_image_id_index = real_image_src.indexOf("image_id=")
    console.log(real_image_id_index)
    let image_id_value = parseInt(real_image_src.substring(real_image_id_index + 9, real_image_src.length))
    console.log(image_id_value)
    activeFetchDetectionDecode(image_id_value)
}
//整体方法
            ...
            <div class="items-center justify-center mt-8 mb-12">
                <div>
                    <div class="w-3/12 inline-block">
                        <div class="mt-3 ml-3 bg-white rounded-2xl shadow-xl">
                            <img class="w-auto h-auto rounded-2xl" src="../../assets/people_1.jpg"
                                ref="active_detection_real" @click="agImgRepo" />
                        </div>
                    </div>
                    <div class="w-2/12 inline-block">
                        <div class="rounded-2xl place-content-center place-items-center">
                            <button data-modal-target="active-defense-modal" data-modal-toggle="active-defense-modal"
                                ref="active_detection_image_repo"
                            ...
                            <div class="mb-10 rounded-2xl place-content-center place-items-center">
                    ...
                    <div class="w-2/12 inline-block">
                        <div class="mb-20 rounded-2xl place-content-center place-items-center pl-12">
                            <img @click="activeDetectionEncode" class="w-full h-auto rounded-2xl"
                                src="../../assets/active_detection_encode.png" />
                        </div>
                    </div>
                    <div class="w-2/12 inline-block">
                        <div class="mb-20 rounded-2xl place-content-center place-items-center pl-12">
                ...
                <div>
                    <div class="w-8/12 inline-block ml-3">
                        ...
                        <textarea id="message" rows="16" ref="active_detection_msgbox"
                    ...
                    <div class="inline-block" style="width:32%">
                        <div class="mt-3 rounded-2xl ml-32 mr-3">
                            <img @click="activeDetectionDecode" class="w-full h-auto rounded-2xl"
                                src="../../assets/active_detection_decode.png" />
                        </div>
                    </div>
                </div>
            </div>
