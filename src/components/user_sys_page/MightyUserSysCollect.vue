...
const base64ImgtoFile = (dataurl, filename = 'photo') => {
...
const fetchUpload = async (file_form_data) => {
    try {
        await axios.post(
            "/user/file_upload", file_form_data, {
            headers: {
                "Content-Type": "multipart/form-data"
            },
        }).then(res => {
            console.log(res)
            initMightyImageRepo(currentPage.value, 8)           //请求成功后执行函数
        }).catch(err => {
            console.log("文件上传失败")
        })
    } catch (error) {
...
const fetchPhotoUpload = async (file_form_data) => {
    try {
        await axios.post(
            "/user/photo_upload", file_form_data, {
            headers: {
                "Content-Type": "multipart/form-data"
            },
        }).then(res => {
...
function mightyUpload(event) {
    let input = document.createElement('input')
    input.type = 'file'
    input.accept = '.png, .jpg'; // 限制选择的文件类型为 .pdf, .png, .zip, .jpg
...
const submitImg = () => {
    ...
    // const imgFile = base64ImgtoFile(photo.value.src);
    // formData.append('file', imgFile)
    // fetchPhotoUpload(formData)
    // // 如果使用formdata，那么接口需要添加 headers: {'Content-Type': 'multipart/form-data'} 
    // // 显示图片
    imageSrc.value = photo.value.src
    ...
    cropper.getCropperSelection().aspectRatio = 1
    cropper.getCropperCanvas().style = "height:480px;width:640px"
}
const cropImage = () => {
    const canvas = cropper.getCropperSelection().$toCanvas();
    canvas.then(data => {
        console.log(data)
        const croppedImage = data.toDataURL('image/png');
        // 如果使用formData的话
        let formData = new FormData()
        const imgFile = base64ImgtoFile(croppedImage);
        formData.append('file', imgFile)
        fetchPhotoUpload(formData)
    })
}
        ...
        <aside class="h-full m-0.5 flex-col transition-transform" style="width: 41.5%;">
            <div class="m-1 w-full bg-blue-100 rounded-md">
                ...
                <div class="w-1/3 inline-block mb-32">
                    <span
                        class="mt-20 text-lg font-semibold text-gray-600 flex items-center justify-center">拍摄图像并上传</span>
                    <div class="mt-5 bg-white rounded-2xl shadow-xl">
                        <img class="w-auto h-auto  rounded-2xl" src="../../assets/photo.jpeg"
                            @click="real_camera.click()" />
                        <button data-modal-target="camera-modal" data-modal-toggle="camera-modal" ref="real_camera"
                            ...
                            <div class="relative p-4 w-full max-w-2xl max-h-full">
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <div class="p-4 md:p-5 space-y-4 flex items-center">
                                        ...
                                        <!-- 弹窗 -->
                                        <div ref="dialogVisible" class="flex items-center" title="照片" width="50%">
                                            <div>
                                                <video ref="video" width="640" height="640" muted></video>
                                            </div>
                                        </div>
                                    ...
                                    <div
                                        ...
                                        <button type="button" @click="handleOpen"
                                            class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-md px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">打开相机</button>
                                        <button type="button" @click="submitImg"
                                            class="py-2.5 px-5 ms-3 text-md font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">拍照上传</button>
                                        <button data-modal-target="crop-modal" data-modal-toggle="crop-modal"
                    ...
                    <span
                        class="mt-24 text-lg font-semibold text-gray-600 flex items-center justify-center">直接上传图像文件</span>
                    <div class="mt-2.5 bg-white rounded-2xl shadow-xl">
                        <input class="hidden" type="file" ref="uploadFace" accept=".png,.jpg" />
                        <img @click="mightyUpload" class="w-auto h-auto rounded-2xl"
        ...
        <aside class="h-full m-0.5 flex-col transition-transform" style="width: 41%;">
            <div class="m-1 w-full bg-pink-100 rounded-md">
                ...
                <div class="w-10/12 inline-block">
                    <span
                        class="mt-6 text-3xl font-semibold text-gray-600 flex items-center justify-center">图像库预览</span>
                    <!-- 预览图像 -->
                    <div class="mt-5 bg-pink-100">
                        <div class="w-2/12 inline-block">
                        </div>
                        <div class="w-8/12 inline-block" style="height: 460.8px;width: 460.8px;">
                    ...
                    <div class=" mt-6 bg-gray-300 rounded-xl h-80 flex" id="img_repo_content">
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-8 ml-2.5">
                            <div class="h-36 w-36"><img src="" id="image_box_1" decoding="sync"
                                    class="image_box rounded-xl" @click="changePreview"></div>
                            <div class="h-36 w-36"><img src="" id="image_box_2" decoding="sync"
                                    class="image_box rounded-xl" @click="changePreview"></div>
                            <div class="h-36 w-36"><img src="" id="image_box_3" decoding="sync"
                                    class="image_box rounded-xl" @click="changePreview"></div>
                            <div class="h-36 w-36"><img src="" id="image_box_4" decoding="sync"
                                    class="image_box rounded-xl" @click="changePreview"></div>
                            <div class="h-36 w-36"><img src="" id="image_box_5" decoding="sync"
                                    class="image_box rounded-xl" @click="changePreview"></div>
                            <div class="h-36 w-36"><img src="" id="image_box_6" decoding="sync"
                                    class="image_box rounded-xl" @click="changePreview"></div>
                            <div class="h-36 w-36"><img src="" id="image_box_7" decoding="sync"
                                    class="image_box rounded-xl" @click="changePreview"></div>
                            <div class="h-36 w-36"><img src="" id="image_box_8" decoding="sync"
                                    class="image_box rounded-xl" @click="changePreview"></div>
                        </div>
                    </div>
                    <!-- 分页 -->
    ...
    <div id="crop-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-3xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <div class="p-4 md:p-5 space-y-4" ref="cropperdiv">
                    <div id="cropper-container" class="cropper-container" style="height: 480px;width:640px">
                        <img ref="imageRef" :src="imageSrc" alt="image">
                    </div>
                </div>
                <!-- Modal footer -->
                <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                    <button type="button" @click="cropImage" data-modal-hide="crop-modal"
