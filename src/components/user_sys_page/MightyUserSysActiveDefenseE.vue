...
const aggan_psnr_sod = ref(null)
const aggan_ssim_sod = ref(null)
const aggan_l2_mask = ref(null)
const aggan_sr_mask = ref(null)
const fgan_psnr_sod = ref(null)
const fgan_ssim_sod = ref(null)
const fgan_l2_mask = ref(null)
const fgan_sr_mask = ref(null)
const attgan_psnr_sod = ref(null)
const attgan_ssim_sod = ref(null)
const attgan_l2_mask = ref(null)
const attgan_sr_mask = ref(null)
const stargan_psnr_sod = ref(null)
const stargan_ssim_sod = ref(null)
const stargan_l2_mask = ref(null)
const stargan_sr_mask = ref(null)
const psnr = ref(null)
const ssim = ref(null)
const l2 = ref(null)
const l1 = ref(null)
...
function mightySocketMessage(event) {
    ...
    if (received_msg.sender_type == "active_defense_eval") {
        active_defense_msgbox.value.value = active_defense_msgbox.value.value + received_msg.sender_msg + "\n"
    } else if (received_msg.sender_type == "active_defense_eval_result") {
        ...
        aggan_psnr_sod.value.innerText = result.aggan_psnr_sod
        aggan_ssim_sod.value.innerText = result.aggan_ssim_sod
        aggan_l2_mask.value.innerText = result.aggan_l2_mask
        aggan_sr_mask.value.innerText = result.aggan_sr_mask + "%"

        fgan_psnr_sod.value.innerText = result.fgan_psnr_sod
        fgan_ssim_sod.value.innerText = result.fgan_ssim_sod
        fgan_l2_mask.value.innerText = result.fgan_l2_mask
        fgan_sr_mask.value.innerText = result.fgan_sr_mask + "%"

        attgan_psnr_sod.value.innerText = result.attgan_psnr_sod
        attgan_ssim_sod.value.innerText = result.attgan_ssim_sod
        attgan_l2_mask.value.innerText = result.attgan_l2_mask
        attgan_sr_mask.value.innerText = result.attgan_sr_mask + "%"

        stargan_psnr_sod.value.innerText = result.stargan_psnr_sod
        stargan_ssim_sod.value.innerText = result.stargan_ssim_sod
        stargan_l2_mask.value.innerText = result.stargan_l2_mask
        stargan_sr_mask.value.innerText = result.stargan_sr_mask + "%"
    }
}
const fetchCustom = async (file_form_data) => {
    try {
        await axios.post(
            "/user/active_defense_eval_custom", file_form_data, {
            headers: {
...
const fetchDefault = async (file_form_data) => {
    try {
        await axios.post(
            "/user/active_defense_eval_default", file_form_data, {
            headers: {
                "Content-Type": "multipart/form-data"
            },
        }).then(res => {
        ...
        <aside class="h-full m-0.5 flex-col transition-transform" style="width: 41.5%;">
            <div class="m-1 w-full bg-blue-100 rounded-md">
                ...
                <div class="w-1/3 inline-block mb-32">
                    <span
                        class="mt-20 text-lg font-semibold text-gray-600 flex items-center justify-center">使用已有的数据集进行评估</span>
                    ...
                    <span
                        class="mt-24 text-lg font-semibold text-gray-600 flex items-center justify-center">上传构建好的数据集进行评估</span>
                    <div class="mt-3 bg-white rounded-2xl shadow-xl">
        ...
        <aside class="h-full m-0.5 flex-col transition-transform" style="width: 41%;">
            <div class="m-1 w-full bg-pink-100 rounded-md">
                ...
                <div class="w-10/12 inline-block">
                    <span
                        class="mt-2 text-3xl font-semibold text-gray-600 flex items-center justify-center">主动防御性能评估数据预览</span>
                    <!-- 表格 -->
                                ...
                                <table class="w-full text-md text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    ...
                                    <tbody>
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            ...
                                            <td class="px-6 py-4" ref="aggan_psnr_sod">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="aggan_ssim_sod">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="aggan_l2_mask">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="aggan_sr_mask">
                                                暂无
                                            </td>
                                        </tr>
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            ...
                                            <td class="px-6 py-4" ref="fgan_psnr_sod">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="fgan_ssim_sod">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="fgan_l2_mask">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="fgan_sr_mask">
                                                暂无
                                            </td>
                                        </tr>
                                        <tr class="bg-white dark:bg-gray-800">
                                            ...
                                            <td class="px-6 py-4" ref="attgan_psnr_sod">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="attgan_ssim_sod">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="attgan_l2_mask">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="attgan_sr_mask">
                                                暂无
                                            </td>
                                        </tr>
                                        <tr class="bg-white dark:bg-gray-800">
                                            ...
                                            <td class="px-6 py-4" ref="stargan_psnr_sod">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="stargan_ssim_sod">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="stargan_l2_mask">
                                                暂无
                                            </td>
                                            <td class="px-6 py-4" ref="stargan_sr_mask">
                                                暂无
                                            </td>
                                        </tr>
                                    </tbody>
                    ...
                    <span
                        class="mt-8 text-3xl font-semibold text-gray-600 flex items-center justify-center">图像视觉质量评估数据预览</span>
                    <!-- 表格 -->
                    <div class="mt-3 bg-pink-100">
                        <div class="w-full mb-9 inline-block">
                            <div class="relative overflow-x-auto">
                                <table class="w-full text-md text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    <thead
                                        class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
