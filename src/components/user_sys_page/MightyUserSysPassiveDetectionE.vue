...
const passive_defense_msgbox = ref(null)
let websocket_url = "ws://localhost:3407/mighty_socket"
let token = getToken()
let mighty_socket = null
const dataset_names = ref(null)
const imageRef = ref(null)
const defense_data_name = ref(null)
const defense_acc = ref(null)
const defense_ap = ref(null)
...
function mightySocketMessage(event) {
    ...
    if (received_msg.sender_type == "passive_defense_eval") {
        passive_defense_msgbox.value.value = passive_defense_msgbox.value.value + received_msg.sender_msg + "\n"
    } else if (received_msg.sender_type == "passive_defense_eval_result") {
        const result = received_msg.sender_msg
        defense_acc.value.innerText = result.acc + "%"
        defense_ap.value.innerText = result.ap + "%"
    }
}
const fetchCustom = async (file_form_data) => {
    defense_data_name.value.innerText = "自定义数据集"
    try {
        await axios.post(
            "/user/passive_defense_eval_custom", file_form_data, {
            headers: {
...
const fetchDefault = async () => {
    defense_data_name.value.innerText = dataset_names.value.selectedOptions[0].value
    try {
        await axios({
            method: "post",
            url: '/user/passive_defense_eval_default',
            headers: {
                "Content-Type": "application/json"
            },
            data: JSON.stringify({
                dataset_name: dataset_names.value.selectedOptions[0].value,
            })
        }).then(res => {
            console.log(res)
        }).catch(err => {
    ...
    <div class="flex w-full h-full flex-row">
        ...
        <aside class="h-full m-0.5 flex-col transition-transform" style="width: 41.5%;">
            <div class="m-1 w-full bg-blue-100 rounded-md">
                ...
                <div class="w-1/3 inline-block mb-80">
                    <form class="max-w-md mx-auto">
                        <label for="dataset_names"
                            class="mt-20 block mb-2 text-lg font-medium text-gray-900 dark:text-white">使用已有的数据集进行评估</label>
                        <select id="dataset_names" ref="dataset_names"
                            class="bg-gray-50 border inline-block border-gray-300 text-gray-900 w-2/3 text-base rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option selected>选择测试的数据集</option>
                            <option value="biggan">biggan</option>
                            <option value="cyclegan">cyclegan</option>
                            <option value="deepfake">deepfake</option>
                            <option value="gaugan">gaugan</option>
                            <option value="progan">progan</option>
                            <option value="aggan">aggan</option>
                            <option value="fgan">fgan</option>
                            <option value="attgan">attgan</option>
                            <option value="stargan">stargan</option>
                            <option value="stylegan">stylegan</option>
                            <option value="stylegan2">stylegan2</option>
                            <option value="began">began</option>
                            <option value="cramergan">cramergan</option>
                            <option value="infomaxgan">infomaxgan</option>
                            <option value="mmdgan">mmdgan</option>
                            <option value="relgan">relgan</option>
                            <option value="s3gan">s3gan</option>
                            <option value="sngan">sngan</option>
                            <option value="adm">adm</option>
                            <option value="ddpm">ddpm</option>
                            <option value="iddpm">iddpm</option>
                            <option value="ldm">ldm</option>
                            <option value="pndm">pndm</option>
                            <option value="sdv1_new">sdv1_new</option>
                            <option value="sdv2">sdv2</option>
                            <option value="vqdiffusion">vqdiffusion</option>
                            <option value="dalle">dalle</option>
                            <option value="glide_100_10">glide_100_10</option>
                            <option value="glide_100_27">glide_100_27</option>
                            <option value="glide_50_27">glide_50_27</option>
                            <option value="guided">guided</option>
                            <option value="ldm_100">ldm_100</option>
                            <option value="ldm_200">ldm_200</option>
                            <option value="ldm_200_cfg">ldm_200_cfg</option>
                            <option value="1k_dalle">1k_dalle</option>
                            <option value="1k_ddpm">1k_ddpm</option>
                            <option value="1k_guided">1k_guided</option>
                            <option value="1k_iddpm">1k_iddpm</option>
                            <option value="midjourney">midjourney</option>
                        </select>
                        <button
                            class="middle none center rounded-lg bg-sky-500 py-2 px-6 ml-3 font-sans text-base font-bold uppercase text-white inline-block"
                            @click="mightyDefault">
                            确认
                        </button>
                    </form>
                    ...
                    <div class="w-1/12 inline-block mt-12"></div>
                </div>
                <div class="w-1/3 inline-block"></div>
            </div>
        </aside>
        <aside class="h-full m-0.5 flex-col transition-transform" style="width: 41%;">
            <div class="m-1 w-full bg-pink-100 rounded-md">
                ...
                <div class="w-10/12 inline-block">
                    <span
                        class="mt-8 text-3xl font-semibold text-gray-600 flex items-center justify-center">被动防御检测准确率与精度预览</span>
                    <!-- 表格 -->
                    <div class="mt-5 mb-10 bg-pink-100">
                                ...
                                <table class="w-full text-md text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    ...
                                    <tbody>
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            <th scope="col" class="px-6 py-3" ref="defense_data_name">
                                                暂无
                                            </th>
                                            <th scope="col" class="px-6 py-3" ref="defense_acc">
                                                暂无
                                            </th>
                                            <th scope="col" class="px-6 py-3" ref="defense_ap">
                                                暂无
                                            </th>
                                        </tr>
                                    </tbody>
                    ...
                    <textarea id="message" rows="24" ref="passive_defense_msgbox"
                    ...
                    <div class="w-1/12 inline-block mt-28 mb-2"></div>
                </div>
                <div class="w-1/12 inline-block"></div>
            </div>
        </aside>
